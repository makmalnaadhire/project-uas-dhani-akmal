    ...
    <div className="pt-24 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-screen">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold font-[family-name:var(--font-display)] mb-2">
          <span className="text-gradient-mint">Katalog</span> & Perbandingan
        </h1>
        <p className="text-slate-400 text-sm">
          Jelajahi {laptops.length} laptop — klik kartu untuk detail, atau pilih untuk perbandingan.
        </p>
      </div>
      ...
      <div className="glass rounded-xl p-4 mb-6">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              placeholder="Cari laptop, brand, CPU, GPU..."
              value={search}
      ...
      {compareList.length > 0 && (
        <div className="glass rounded-xl p-4 mb-6 flex items-center justify-between border-[#2dd4bf]/20">
          <span className="text-sm text-[#2dd4bf] font-medium">
            {compareList.length}/3 dipilih untuk perbandingan
          </span>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-slate-400 text-lg mb-2">Tidak ada laptop ditemukan</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 stagger">
          {filtered.map(laptop => {
            ...
            return (
              <div
                key={laptop.id}
                className="card-glow glass rounded-xl overflow-hidden cursor-pointer group"
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ml-2 whitespace-nowrap ${
                  ...
                  {/* Category */}
                  <span
                    className={`inline-block text-[10px] font-medium px-2 py-0.5 rounded-md border mb-4 ${
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={e => { e.stopPropagation(); toggleWishlist(laptop.id); }}
                        className={`p-2 rounded-lg transition-all ${
                          isWished
                            ? "bg-[#ec4899]/15 text-[#ec4899]"
                            : "bg-white/5 text-slate-500 hover:text-[#ec4899] hover:bg-[#ec4899]/10"
                        }`}
                      >
                        <Heart size={14} fill={isWished ? "currentColor" : "none"} />
                      </button>
                      <button
                        onClick={e => { e.stopPropagation(); toggleCompare(laptop); }}
                        className={`px-2.5 py-1.5 rounded-lg text-[10px] font-medium transition-all ${
                          inCompare
                            ? "bg-[#2dd4bf]/15 text-[#2dd4bf] border border-[#2dd4bf]/30"
                            : "bg-white/5 text-slate-500 border border-white/10 hover:text-[#2dd4bf] hover:border-[#2dd4bf]/30"
                        }`}
                      >
                        {inCompare ? "✓ Bandingkan" : "Bandingkan"}
                      </button>
                    </div>
                  </div>
                </div>
