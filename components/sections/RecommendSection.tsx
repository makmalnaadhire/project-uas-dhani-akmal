...
export default function RecommendSection({ laptops, switchTab, setSelectedLaptop }: Props) {
  ...
  const results = useMemo(() => {
    ...
    return laptops.filter(l => {
      ...
      // Score: prefer New, then higher RAM, then lower price
      let sa = 0, sb = 0;
      if (a.condition === "New") sa += 10;
      if (b.condition === "New") sb += 10;
      const ra = parseInt(a.ram);
      const rb = parseInt(b.ram);
      sa += ra;
      sb += rb;
    ...
    <div className="pt-24 pb-20 max-w-3xl mx-auto px-4 sm:px-6 min-h-screen">
      ...
      <div className="glass rounded-xl p-6 sm:p-8 min-h-[320px]">
        {step === 1 && (
          <div className="slide-up">
            <h2 className="text-xl font-semibold font-[family-name:var(--font-display)] mb-2">
              ...
              {needs.map(n => (
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl border text-left transition-all ${
                    selectedNeed === n.id
                      ? "bg-[#f97316]/10 border-[#f97316]/40 shadow-[0_0_15px_rgba(249,115,22,0.1)]"
                      : "bg-white/3 border-white/10 hover:border-white/20"
                  }`}
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    selectedNeed === n.id ? "bg-[#f97316]/20 text-[#f97316]" : "bg-white/5 text-slate-500"
                  }`}>
                    {n.icon}
                  </div>
                  <div>
        ...
        {step === 2 && (
          <div className="slide-up">
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-between mt-2">
                {budgetRanges.map((b, i) => (
                  <span key={i} className={`text-[10px] ${i === budgetRange ? "text-[#f97316] font-medium" : "text-slate-600"}`}>
                    {b.label}
                  </span>
                ))}
              </div>
            ...
            <div className="text-center p-4 rounded-lg bg-[#f97316]/10 border border-[#f97316]/20">
              ...
              <p className="text-xl font-bold text-[#f97316] font-[family-name:var(--font-display)]">
          ...
          <div className="slide-up">
            ...
            <div className="grid gap-3">
              {conditions.map(c => (
                ...
                >
                  <div className={`text-sm font-medium ${selectedCondition === c.id ? "text-[#f97316]" : "text-white"}`}>
                    {c.label}
                  </div>
          ...
          <div className="slide-up">
            ...
            {results.length === 0 ? (
              <div className="text-center py-8">
                ...
                <button
                  onClick={() => { setStep(1); setSelectedNeed(""); }}
              ...
              <div className="space-y-3">
                {results.slice(0, 6).map((l, i) => (
                  <button
                    ...
                    className="w-full flex items-center gap-4 p-4 rounded-xl bg-white/3 border border-white/10 hover:border-[#f97316]/30 text-left transition-all group"
                  >
                    <div className="w-8 h-8 rounded-lg bg-[#f97316]/10 text-[#f97316] flex items-center justify-center text-sm font-bold flex-shrink-0">
                      {i + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-white group-hover:text-[#f97316] transition-colors truncate">
                        {l.name}
                      </div>
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <div className={`text-[10px] ${l.condition === "New" ? "text-[#2dd4bf]" : "text-[#f97316]"}`}>
                        {l.condition === "New" ? "Baru" : "Bekas"}
                      </div>
                    </div>
      ...
      <div className="flex justify-between mt-6">
        <button
          ...
          disabled={step === 1}
          className="flex items-center gap-1 px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
        >
          <ChevronLeft size={16} />
          Kembali
        </button>
        {step < 4 ? (
          <button
            ...
            disabled={(step === 1 && !selectedNeed)}
            className="flex items-center gap-1 px-5 py-2.5 rounded-lg bg-gradient-to-r from-[#f97316] to-[#ef4444] text-white text-sm font-semibold hover:shadow-[0_0_20px_rgba(249,115,22,0.3)] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
          >
          ...
          <button
            onClick={() => { setStep(1); setSelectedNeed(""); setBudgetRange(1); setSelectedCondition("All"); }}
            className="flex items-center gap-1 px-5 py-2.5 rounded-lg bg-[#f97316]/10 border border-[#f97316]/30 text-[#f97316] text-sm font-medium hover:bg-[#f97316]/20 transition-all"
          >
            Mulai Ulang
          </button>
        )}
